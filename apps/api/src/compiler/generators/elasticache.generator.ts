...
import type { CanvasNode, TerraformReference, ElastiCacheConfig } from '@canvascloud/shared';
...
export class ElastiCacheGenerator extends BaseGenerator {
  ...
  generate(node: CanvasNode, references: TerraformReference[]): string {
    ...
    const sgRefs = this.findReferences(node.id, 'security_group_ids', references);
    ...
    const subnetGroupVal = subnetRefs.length > 0
      ? `aws_elasticache_subnet_group.${name}_subnet_group.name`
      : (config.subnet_group_name ? `"${config.subnet_group_name}"` : null);
    ...
    parts.push(`  node_type            = "${config.node_type || 'cache.t3.micro'}"`);
    ...
    if (config.snapshot_retention_limit !== undefined) {
      parts.push(`  snapshot_retention_limit = ${config.snapshot_retention_limit}`);
    }
    if (config.at_rest_encryption_enabled !== undefined) {
      parts.push(`  at_rest_encryption_enabled = ${config.at_rest_encryption_enabled}`);
    }
    if (config.transit_encryption_enabled !== undefined) {
      parts.push(`  transit_encryption_enabled = ${config.transit_encryption_enabled}`);
    }
