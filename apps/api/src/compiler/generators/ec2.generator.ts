...
export class EC2Generator extends BaseGenerator {
  ...
  generate(node: CanvasNode, references: TerraformReference[]): string {
    ...
    const iamRef = references.find(
      (r) =>
        r.targetNodeId === node.id &&
        r.isSynthetic &&
        r.terraformExpression === 'aws_iam_instance_profile',
    );
    if (iamRef) {
      parts.push(
        `  iam_instance_profile = aws_iam_instance_profile.${name}_profile.name`,
      );
    }
    ...
    if (config.root_block_device) {
      ...
      if (config.root_block_device.volume_type) {
        parts.push(
          `    volume_type = "${config.root_block_device.volume_type}"`,
        );
      }
      ...
      if (config.root_block_device.delete_on_termination !== undefined) {
        parts.push(
          `    delete_on_termination = ${config.root_block_device.delete_on_termination}`,
        );
      }
      parts.push(`  }`);
    }
