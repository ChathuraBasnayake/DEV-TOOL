...
import type { CanvasNode, TerraformReference, NATGWConfig } from '@canvascloud/shared';
...
export class NATGWGenerator extends BaseGenerator {
  ...
  generate(node: CanvasNode, references: TerraformReference[]): string {
    ...
    const allocationIdVal = eipRef ? eipRef.terraformExpression : (config.allocation_id ? `"${config.allocation_id}"` : null);
