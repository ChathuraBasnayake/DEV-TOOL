...
export class NoNATRule extends BaseGuardrailRule {
  ...
  readonly description = 'Private subnets lack outbound internet routing via a NAT Gateway';
    ...
    for (const subnet of privateSubnets) {
      ...
      if (!hasNAT || !rtHasNATRoute) {
        warnings.push({
          ...
          suggestion: 'Create a NAT Gateway in a public subnet and route traffic from this private Route Table to the NAT Gateway.',
        });
      }
    }
