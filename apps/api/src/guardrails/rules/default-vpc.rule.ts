...
export class DefaultVPCRule extends BaseGuardrailRule {
  ...
  readonly description = 'Resources deploy into default VPC/network space without custom subnet segmentation';
  ...
  evaluate(nodes: CanvasNode[], edges: CanvasEdge[]): SecurityWarning[] {
    ...
    const isolatedNodes = nodes.filter(n => targetTypes.includes(n.data.resourceType) && !subnetConnectedNodeIds.has(n.id));
    ...
    for (const node of isolatedNodes) {
      warnings.push({
        ...
        suggestion: 'Draw a connection from a Subnet node to this resource to deploy it in a designated VPC network.',
      });
    }
