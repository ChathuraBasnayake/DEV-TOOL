...
export class NoLambdaRoleRule extends BaseGuardrailRule {
  ...
  readonly description = 'Lambda function deployed without an associated IAM execution role';
  ...
  evaluate(nodes: CanvasNode[], edges: CanvasEdge[]): SecurityWarning[] {
    ...
    for (const lambda of lambdaNodes) {
      ...
      const hasRole = edges.some(e => {
        ...
        return sourceNode && sourceNode.data.resourceType === 'iam-role' && e.target === lambda.id;
      });
      ...
      if (!hasRole) {
        warnings.push({
          ...
          suggestion: 'Create an IAM Role and draw a connection to this Lambda function.',
        });
      }
    }
