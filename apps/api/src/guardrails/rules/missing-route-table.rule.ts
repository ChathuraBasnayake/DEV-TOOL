...
export class MissingRouteTableRule extends BaseGuardrailRule {
  ...
  readonly description = 'Subnet does not have any Route Table associated with it';
  ...
  evaluate(nodes: CanvasNode[], edges: CanvasEdge[]): SecurityWarning[] {
    ...
    for (const subnet of subnetNodes) {
      ...
      const isAssociated = edges.some(e => {
        ...
        return sourceNode && sourceNode.data.resourceType === 'route-table' && e.target === subnet.id;
      });
      ...
      if (!isAssociated) {
        warnings.push({
          ...
          suggestion: 'Create a Route Table node and connect it to this Subnet to define traffic routes.',
        });
      }
    }
