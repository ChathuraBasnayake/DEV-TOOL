...
export class ASGNoHealthCheckRule extends BaseGuardrailRule {
  ...
  readonly description =
    'Auto Scaling Group connected to Target Group is not configured to use ELB health checks';
  ...
  evaluate(nodes: CanvasNode[], edges: CanvasEdge[]): SecurityWarning[] {
    ...
    for (const asg of asgNodes) {
      ...
      const hasTG = edges.some((e) => {
        ...
        return (
          targetNode &&
          targetNode.data.resourceType === 'target-group' &&
          e.source === asg.id
        );
      });
      ...
      if (!config || config.health_check_type !== 'ELB') {
        warnings.push({
          ...
          suggestion:
            'Set health_check_type to "ELB" to ensure scaling actions respect load balancer health checks.',
        });
      }
    }
