...
export class OpenSGRule extends BaseGuardrailRule {
  ...
  readonly description = 'Inbound security group rules allow access to sensitive ports from any IP address';
      ...
      for (const rule of config.ingressRules) {
        ...
        if (exposedSensitivePorts.length > 0) {
          warnings.push({
            ...
            suggestion: 'Restrict ingress CIDRs to specific trusted IP ranges or reference specific Security Groups.',
          });
        }
      }
    }
