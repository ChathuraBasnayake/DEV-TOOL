...
import type { CanvasNode, CanvasEdge, SecurityWarning, S3Config, RDSConfig, DynamoDBConfig } from '@canvascloud/shared';
...
export class NoEncryptionRule extends BaseGuardrailRule {
  ...
  readonly description = 'Resources holding persistent data lack server-side encryption';
        ...
        case 's3': {
          ...
          if (!config || !config.sse_algorithm) {
            warnings.push({
              ...
              suggestion: 'Set sse_algorithm (e.g. "AES256") to enforce encryption on new objects.',
            });
          }
          break;
        }
        ...
        case 'rds': {
          ...
          if (!config || config.storage_encrypted !== true) {
            warnings.push({
              ...
              suggestion: 'Set storage_encrypted to true to enable KMS-managed volume encryption.',
            });
          }
          break;
        }
        ...
        case 'dynamodb': {
          ...
          if (!config || config.server_side_encryption !== true) {
            warnings.push({
              ...
              suggestion: 'Set server_side_encryption to true to protect table data at rest.',
            });
          }
          break;
        }
