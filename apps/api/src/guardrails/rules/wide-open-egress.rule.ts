...
import type { CanvasNode, CanvasEdge, SecurityWarning, SecurityGroupConfig } from '@canvascloud/shared';
...
export class WideOpenEgressRule extends BaseGuardrailRule {
  ...
  readonly description = 'Security Group configuration allows unrestricted outbound traffic to the public internet';

  evaluate(nodes: CanvasNode[], edges: CanvasEdge[]): SecurityWarning[] {
    const warnings: SecurityWarning[] = [];
    const sgNodes = nodes.filter(n => n.data.resourceType === 'security-group');
    ...
    for (const node of sgNodes) {
      ...
      for (const rule of config.egressRules) {
        const isWideOpen = rule.cidr_blocks?.some(cidr => cidr === '0.0.0.0/0' || cidr === '::/0');
        if (!isWideOpen) continue;

        // Protocol "-1" means all traffic, or ports covering full range
        const isAllPorts = rule.protocol === '-1' || (rule.from_port === 0 && rule.to_port === 0) || (rule.from_port === 0 && rule.to_port === 65535);
        ...
        if (isAllPorts) {
          warnings.push({
            ...
            suggestion: 'Restrict egress traffic to the specific ports and CIDR blocks required by your application.',
          });
          break;
        }
      }
