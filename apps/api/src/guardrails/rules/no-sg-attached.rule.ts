...
export class NoSGAttachedRule extends BaseGuardrailRule {
  ...
  readonly description =
    'Resources deploy without any security group attachments, leaving firewall settings open to defaults';
  ...
  evaluate(nodes: CanvasNode[], edges: CanvasEdge[]): SecurityWarning[] {
    ...
    const unshieldedNodes = nodes.filter(
      (n) =>
        targets.includes(n.data.resourceType) && !sgConnectedNodeIds.has(n.id),
    );
    ...
    for (const node of unshieldedNodes) {
      warnings.push({
        ...
        suggestion:
          'Create a Security Group node and draw a connection to this resource to control incoming traffic.',
      });
    }
